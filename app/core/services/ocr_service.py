# app/core/services/ocr_service.py
...
from app.adapters.interfaces.ocr_engine import OcrBox, TextOCREngine, FormulaOCREngine
...
# OpenCV / PIL helpers (взяты из монолитного модуля)
...
# Helpers for grouping boxes -> markdown
_MATH_BLOCK_RE = re.compile(r"(\$\$.*?\$\$|\$(?:\\.|[^$\\])+\$|\\\[.*?\\\]|\\\(.*?\\\))", re.DOTALL)
_LAT2CYR = str.maketrans({
    "a":"а","A":"А","o":"о","O":"О","e":"е","E":"Е","c":"с","C":"С","p":"р","P":"Р",
    "x":"х","X":"Х","y":"у","Y":"У","k":"к","K":"К","m":"м","M":"М","t":"т","T":"Т",
    "H":"Н","B":"В",
})
_SIMPLE_TEXT_IN_MATH_RE = re.compile(r"^\\mathrm\{([A-Za-zА-Яа-я0-9\s~]+)\}$")
...
# Главный OcrService — композирует FormulaOCREngine + TextOCREngine
class OCRService:
...
# OpenRouter summarization (перенёс из монолита)
