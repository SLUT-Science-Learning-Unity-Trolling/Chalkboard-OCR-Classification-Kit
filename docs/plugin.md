...
```python
def parse_google_docstring(docstring: str):
    """
    Парсит Google-style докстринг в структуру словаря.
    ...
    """
    if not docstring:
        return {"first_line": "", "rest_description": "", "args": "", "returns": "", "raises": ""}

    sections = {"first_line": "", "rest_description": [], "args": [], "returns": [], "raises": []}
...
def extract_docstrings(file_path: Path):
    ...
    docstrings = {"module": ast.get_docstring(tree), "classes": {}}
...
```python
def format_function_md(name: str, doc: dict, is_method=False):
    """
    Форматирует функцию или метод в Markdown с таблицами аргументов, возвращаемых значений и исключений.
...
```python
def write_md(file_path: Path, docstrings: dict):
    """
    Генерирует Markdown-файл для модуля или класса.
    ...
    for cls_name, cls_doc in docstrings.get("classes", {}).items():
        ...
        for method_name, method_doc in cls_doc.get("methods", {}).items():
            md_content.append(format_function_md(method_name, method_doc, is_method=True))
            md_content.append("---")
...
```python
def create_docs(src_dirs: list[Path], dst_dir: Path):
    """
    Создает Markdown-документацию для всех Python-файлов из списка директорий.
    ...
    for src_dir in src_dirs:
        for root, dirs, files in os.walk(src_dir):
            ...
            for file in files:
                ...
                if file_path.suffix in SUPPORTED_EXT and file_path.name != "__init__.py":
...
```python
def push_to_wiki(docs_dir: Path):
    """
    Копирует Markdown-документы в локальный клон Wiki и пушит изменения в удаленный репозиторий.
    ...
    if status.stdout.strip():
        subprocess.run(
            ["git", "-C", str(LOCAL_WIKI_DIR), "commit", "-m", "[Wiki] Обновление документации"],
            check=True
        )
